...
-   After you make a block that sets a variable, use the menu in that variable block to change which variable it refers to. You can also use that menu to change the variable's name.
-   Make the whole "line of code" for `die1` first. Then click the `set die1 to` block to select it. **Copy** and then **Paste**. Shazam! You get a copy *of the whole line*. Simply change it to `die2` and position it by dragging that initial block.
