...
# More about tree quality: an example with breast cancer data {#breast-cancer-chapter}
...
Let us begin by saying that the two ways you can be wrong---`FN` and `FP`---are
...
There are other measurements too, such as `perimeter`, which is obvious,
...
you already have a tree set to predict `biopsy`.
...
([Learn more about the configuration box](90-arbor-configuration.qmd).)

::: {.column-margin}
![](art/breastCancer-configuration.png){ width=60%}
:::
...
If your setup is like mine, that will give you 125 malignant diagnoses, of which 
122 were correct. 
So that leaves three false positives. 
On the other hand, there are 90 false negatives. 
That's a lot!
...
[just like we did in the previous section](50-arbor-measures.qmd).
...
* Make a new attribute (perhaps `rating`) and give it that formula, `FN * 5 + FP`.
The value you've calculated appears in the table. 

* Make another new attribute (perhaps `cutpoint`) 
and enter the value for the cut point (otherwise you might forget).

Your summary table might look like this:

![Our "emitted data" table, with two new columns at the far right.](art/arbor-rating-cutpoint-table.png){width=80%}
